...
import { getGuilds } from "./core/discord/api";
...
export default {
  ...
  methods: {
    async loaded() {
      this.listeners.appSavedAuthorizations = await listen("saved-authorizations", (ev) => {
      ...
      this.listeners.discordUserInfo = await listen("user-info", (ev) => {
        try {
          ...
          console.debug("%cListener current-user-info", this.cbd("#f78202"), ev);
        } catch (err) {
          console.error("[listen | user-info]", err);
        }
      });

      this.listeners.discordOnChannelCreate = await listen("discord-channel-create", (ev) => {
        console.debug("%cListener discord-channel-create", this.cbd("#7282c2"), ev);
        useAppStore().data.channels.push(ev.payload);
      });

      this.listeners.discordOnChannelUpdate = await listen("discord-channel-update", (ev) => {
        console.debug("%cListener discord-channel-update", this.cbd("#a282c2"), ev);
        const targetChannel = useAppStore().data.channels.findIndex((channel) => channel.id === ev.payload.new.id);
        if (targetChannel !== -1) {
          useAppStore().data.channels[targetChannel] = ev.payload.new;
        }
      });

      this.listeners.discordOnChannelDelete = await listen("discord-channel-delete", (ev) => {
        console.debug("%cListener discord-channel-delete", this.cbd("#c582c2"), ev);
        const targetChannel = useAppStore().data.channels.findIndex((channel) => channel.id === ev.payload.id);
        if (targetChannel !== -1) {
      ...
      this.listeners.discordOnMessageReactionAdd = await listen("discord-reaction-add", (ev) => {
        console.debug("%cListener discord-reaction-add", this.cbd("#298202"), ev);
        if (ev.payload.channel_id === useAppStore().data.currentChannelId) {
      ...
      this.listeners.discordOnMessageUpdate = await listen("discord-message-update", (ev) => {
        console.debug("%cListener discord-message-update", this.cbd("#828202"), ev);
      ...
      this.listeners.discordOnMessageDelete = await listen("discord-message-delete", (ev) => {
        console.debug("%cListener discord-message-delete", this.cbd("#fa8202"), ev);
      ...
      this.listeners.discordOnPresenceUpdate = await listen("discord-presence-update", (ev) => {
        console.debug("%cListener discord-presence-update", this.cbd("#af82af"), ev);
